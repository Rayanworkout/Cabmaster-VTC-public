...
def get_distance(origin, destination):
    ...
    if TEST_MODE is True:
        return {
            ...
            "duration": "20 minutes",
            "origin": "test address",
            "destination": "test address destination",
        }
...
def price_standard_grade(distance):
    if distance <= 5:
        price = 25
...
def price_berline_grade(distance):
    if distance <= 15:
        price = 50
...
def calculate_price(origin, destination, grade):
    ...
    match grade:
        case "standard":
            price = price_standard_grade(distance_km)

        case "berline":
            price = price_berline_grade(distance_km)

        case "van":
            price = price_van_grade(distance_km)

        case "all":
            price = (
                price_standard_grade(distance_km),
                price_berline_grade(distance_km),
                price_van_grade(distance_km),
            )
...
def clean_reservation_form_data(data):
    ...
    data["payment_mode"] = (
        ...
        if payment_mode == "cash"
            ...
            else "En Ligne" if payment_mode == "online" else "Non renseigné"
        )
    )
